...
def main():
    ...
    cell_size = 60
    pygame.init()
    SURFACE = pygame.display.set_mode((cell_size*(n_ticks+2), cell_size*(n_ticks+2)))
    ...
    while agnt_0.episode < 5:
        ...
        brick = pygame.image.load("./images/Brick_5.png")
        brick = pygame.transform.scale(brick, (cell_size, cell_size))
        
        g_predator = pygame.image.load("./images/g_predator.jpg")
        g_predator = pygame.transform.scale(g_predator, (cell_size, cell_size))
        
        g_predator_2 = pygame.image.load("./images/g_predator_2.png")
        g_predator_2 = pygame.transform.scale(g_predator_2, (cell_size, cell_size))
        
        g_predator_prey = pygame.image.load("./images/g_predator_prey.jpg")
        g_predator_prey = pygame.transform.scale(g_predator_prey, (cell_size, cell_size))
        
        g_prey = pygame.image.load("./images/g_prey.jpg")
        g_prey = pygame.transform.scale(g_prey, (cell_size, cell_size))
        ...
        while not done and ep_step < agnt_0.ep_trial_step:
            ...
            for idx in range(n_ticks+2):
                SURFACE.blit(brick, (cell_size*idx, 0))
                SURFACE.blit(brick, (cell_size*idx, cell_size*(n_ticks+1)))
            for idx in range(n_ticks):
                SURFACE.blit(brick, (0, cell_size*(idx+1)))
                SURFACE.blit(brick, (cell_size*(n_ticks+1), cell_size*(idx+1)))

            # SURFACE.blit(brick, (cell_size+step, 120))
            SURFACE.blit(g_prey, (cell_size*game.prey_row, cell_size*game.prey_col))
            
            SURFACE.blit(g_predator, (cell_size*game.predator_rows[0], cell_size*game.predator_cols[0]))
            SURFACE.blit(g_predator, (cell_size*game.predator_rows[1], cell_size*game.predator_cols[1]))
            SURFACE.blit(g_predator, (cell_size*game.predator_rows[2], cell_size*game.predator_cols[2]))
            SURFACE.blit(g_predator, (cell_size*game.predator_rows[3], cell_size*game.predator_cols[3]))
            
            # SURFACE.blit(logo2, (20+step*2, 150))        
            for row_idx in range((n_ticks-1)):
                pygame.draw.line(SURFACE, (0, 0, 255), (cell_size*(row_idx+2), cell_size*1), (cell_size*(row_idx+2), cell_size*(n_ticks+1)))
            for col_idx in range((n_ticks-1)):
                pygame.draw.line(SURFACE, (0, 0, 255), (cell_size*1, cell_size*(col_idx+2)), (cell_size*(n_ticks+1), cell_size*(col_idx+2)))
